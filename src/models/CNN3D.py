...
class I3D(nn.Module):
    ...
    def forward(self, frame_list, flow_frames,currentFrame,progressLabel,progressBar):
        ...
        with torch.no_grad():
            print("extracting features of rgb")
            progressLabel.config(text='extracting features of rgb')
            progressBar["value"] = 0;
            currentFrame.update()
            ...
            for start in range(t-16):
                ...
                progressBar["value"] = (start / (t-16)) * 100
                currentFrame.update()

            print("extracting features of flow")
            progressLabel.config(text='extracting features of flow')
            progressBar["value"] = 0;
            currentFrame.update()
            ...
            for start in range(t-16):
                ...
                progressBar["value"] = (start / (t-16)) * 100
                currentFrame.update()
        progressLabel.config(text='Processing RBG and FLOW')
        #progressBar["value"] = 0;
        currentFrame.update()
...
class ResNet3D(nn.Module):
    ...
    def forward(self, frame_list,currentFrame,progressLabel,progressBar):
        ...
        with torch.no_grad():
            ...
            features = []
            progressLabel.config(text='extracting features for ResNet3D video')
            progressBar["value"] = 0;
            currentFrame.update()
