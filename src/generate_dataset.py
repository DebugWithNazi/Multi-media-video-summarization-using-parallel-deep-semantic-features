...
class Generate_Dataset:
    def __init__(self, video_path, path_ground_truth, save_path, dataset='custom',
                 ...
                 frame_root_path='..\\frames',save_root_path='..\\datasets\\object_features\\',resnet=True, inception=True, googlenet=True):
        self.progressBar = progressBar
        self.currentFrame = currentFrame
        self.device = torch.device(
            "cuda:" + (os.getenv('N_CUDA') if os.getenv('N_CUDA') else "0") if torch.cuda.is_available() else "cpu")
    ...
    def _extract_video_feature(self, frame_resized, flow_frames,currentFrame,progressLabel,progressBar):
        features_rgb, features_flow = self.video_models["i3d"](frame_resized, flow_frames,currentFrame,progressLabel,progressBar)
        ...
        if self.dataset == 'custom':
            ...
            for video_idx in range(len(self.video_list)):
                ...
                while True:
                    ...
                    img_filename = "{}.jpg".format(str(n_frames+1))
                    print("Current frame image = "+img_filename)
                    cv2.imwrite(os.path.join(self.frame_root_path, video_basename, img_filename), frame)
                    frame_list.append(frame)
                    n_frames += 1
                    if(self.progressBar!=None):
                        print((n_frames/total_frames)*100)
                        self.progressBar["value"]=(n_frames/total_frames)*100
                        self.currentFrame.update()
                # progress["value"] = 21
...
#                flow_frames = np.array(
 #                   [cv2.calcOpticalFlowFarneback(flow_frames[i], flow_frames[i + 15], None, 0.5, 3, 15, 3, 5, 1.2, 0)
  #                   for i in range(len(flow_frames)) if i + 16 <= len(flow_frames)])
                flow_frames1 = []
                ...
                features_rgb, features_flow, features_3D = self._extract_video_feature(frame_resized, flow_frames,self.currentFrame,self.progressLabel,self.progressBar)
            ...
            for video_idx, video_gt in enumerate(tqdm(zip(self.video_list, self.gt_list), total=len(self.video_list))):
                ...
                self.h5_file['video_{}'.format(video_idx + 1)][
                    'user_summary'] = user_score if self.dataset != "tvsum" else np.where(user_score > 0.5, 1, 0)
